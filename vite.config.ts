...
function pwaShellGuard(): Plugin {
    ...
    return {
        ...
        closeBundle() {
            ...
            if (missing.length > 0) {
                throw new Error(
                    `[fm-pwa-shell-guard] these files are missing from ${resolved.build.outDir}/: ` +
                    `${missing.join(', ')}. They are the PWA shell; without them web push cannot ` +
            ...
            if (fs.existsSync(assets) && fs.readdirSync(assets).some((f) => /^sw\..*\.js$/.test(f))) {
                throw new Error(
                    '[fm-pwa-shell-guard] a hashed copy of sw.js was emitted into assets/. The ' +
                    'worker has been pulled into the module graph, which caps its scope at ' +
                    `${resolved.base}assets/ — it can no longer control the app. Keep public/sw.js ` +
                    'out of every import.',
                )
            }
            ...
            if (/addEventListener\s*\(\s*['"`]fetch['"`]/.test(worker)) {
                throw new Error(
                    '[fm-pwa-shell-guard] public/sw.js has grown a `fetch` listener. Read the ' +
            ...
            if (!/rel="manifest"/.test(html)) {
                throw new Error(
                    '[fm-pwa-shell-guard] index.html no longer links the manifest. iOS refuses to ' +
